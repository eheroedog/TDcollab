...
onready var laser = preload("res://Scenes/Towers/LaserTower.tscn")
...
func _input(event):
	...
	if Input.is_action_pressed("add_tower") and just_pressed:
		...
		add_tower = "basic"
	elif Input.is_action_pressed("add_laser") and just_pressed:
		$tower_placement.clear()
		can_place_tower = !can_place_tower
		add_tower = "laser"
...
func _unhandled_input(event):
	if event is InputEventMouseMotion and can_place_tower:
		...
		if not tile in invalid_tiles:
			...
			elif add_tower == "laser":
				var laser_instance = laser.instance()
				#tower_instance.connect("shoot_laser", self, "shoot_laser")
				laser_instance.position = tile * Vector2(32, 32)
				$entities.add_child(laser_instance)
